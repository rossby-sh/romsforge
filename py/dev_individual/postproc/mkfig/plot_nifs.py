...
flist=[Spth+i for i in os.listdir(Spth) if i.startswith('CMEMS_')]
...
for f in flist:
    ...
    with Dataset(f) as ncS:
        ...
        for i in range(len(times)):
            ...
            phytS=ncS['phyc'][i,0].data
            phytS[phytS>1000]=np.nan
            t=times[i]
            t_str=str(num2date(t,'hours since 1950-1-1'))[:10]
            print(t_str)
            draw_roms_pcolor(lon, lat, phytS, timestamp=t_str, varname='Phyt',\
            ...
            chlo[chlo>1000]=np.nan
            draw_roms_pcolor(lon, lat, chlo, timestamp=t_str, varname='chlo',\
                             units='millimole_nitrogen meter-3', log_scale=True, clim=None,\
                                 output_path=wpth+'chl/'+t_str+'.png', cmap=plt.get_cmap('jet',27)) 
