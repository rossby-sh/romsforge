...
PKG_path = 'C:/Users/ust21/shjo\others/will_delet/myROMS/prc_src/utils/' # Location of JNUROMS directory
...
outputFile = 'D:/shjo/tmp_data/obs_KODC_2506.nc'
...
# Table5=pd.read_excel(pth+'202504_KODC_EastSea.xlsx')
# Table6=pd.read_excel(pth+'202504_KODC_WestSea.xlsx')

Table7=pd.read_excel(pth+'202506_KODC_EastSea.xlsx')
Table8=pd.read_excel(pth+'202506_KODC_WestSea.xlsx')
Table9=pd.read_excel(pth+'202506_KODC_SouthSea.xlsx')

Table=pd.concat([Table7,Table8,Table9],axis=0)
...
datetimes = np.array([
    datetime.datetime(year, m, d) - timedelta(hours=9) 
...
for ii in df_sorted.groupby(level=0):
    ...
    obs_error = np.maximum(df_obs["error"].values**2, 1e-3)  # 최소값 제한
    ...
    obs_variance =[0., 0., 0., 0., 0., 0.23122068, 0.0025 , 0., 0., 0., 0.00087414]
