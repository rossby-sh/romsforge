...
flist=[Spth+i for i in os.listdir(Spth) if i.startswith('NWP15_ini_')]
...
for f in flist:
    ...
    with Dataset(f) as ncS:
        ...
        #                      output_path=wpth+'phyt/'+t_str+'.png', cmap=plt.get_cmap('viridis',27)) 
        
        post_phyt=ncS['phytoplankton'][0,-1].data
        post_phyt[post_phyt>10000]=np.nan
        
        draw_roms_pcolor(lon, lat, post_phyt, timestamp='Posterior ('+t_str+')', varname='phyto',\
                         units='millimole_nitrogen meter-3', log_scale=True, clim=None,\
                             output_path=wpth+'post_phyt/'+t_str+'.png', cmap=plt.get_cmap('jet',27)) 

        post_zeta=ncS['zeta'][0].data
        post_zeta[post_zeta==0]=np.nan
        post_zeta[post_zeta>50]=np.nan   

        draw_roms_pcolor(lon, lat, post_zeta, timestamp='Posterior ('+t_str+')', varname='zeta',\
                            units=f'm', log_scale=False, clim=(-1.,2),\
                                output_path=wpth+'post_zeta/'+t_str+'.png',\
                                    cmap=plt.get_cmap('RdBu_r',27) )     

        post_temp=ncS['temp'][0,-1].data
        post_temp[post_temp==0]=np.nan
        post_temp[post_temp>1000]=np.nan

        draw_roms_pcolor(lon, lat, post_temp, timestamp='Posterior ('+t_str+')', varname='Surface temp',\
                            units=f'degree', log_scale=False, clim=(-1,32),\
                                output_path=wpth+'post_temp/'+t_str+'.png',\
                                    cmap=plt.get_cmap('Spectral_r',27) )
        
        post_salt=ncS['salt'][0,-1].data
        post_salt[post_salt==0]=np.nan
        post_salt[post_salt>100]=np.nan    

        draw_roms_pcolor(lon, lat, post_salt, timestamp='Posterior ('+t_str+')', varname='Surface salt',\
                            units=f'psu', log_scale=False, clim=(32.5,35.5),\
                                output_path=wpth+'post_salt/'+t_str+'.png',\
                                    cmap=cmocean.cm.haline) 
        
# -----------------------------------------------------------------------------                

        prior_phyt=ncS['phytoplankton'][1,-1].data
        prior_phyt[prior_phyt>10000]=np.nan
        draw_roms_pcolor(lon, lat, prior_phyt, timestamp='Prior ('+t_str+')', varname='phyto',\
                         units='millimole_nitrogen meter-3', log_scale=True, clim=None,\
                             output_path=wpth+'prior_phyt/'+t_str+'.png', cmap=plt.get_cmap('jet',27)) 

        prior_zeta=ncS['zeta'][1].data
        prior_zeta[prior_zeta==0]=np.nan   
        prior_zeta[prior_zeta>50]=np.nan   

        draw_roms_pcolor(lon, lat, prior_zeta, timestamp='Prior ('+t_str+')', varname='zeta',\
                            units=f'm', log_scale=False, clim=(-1.,2),\
                                output_path=wpth+'prior_zeta/'+t_str+'.png',\
                                    cmap=plt.get_cmap('RdBu_r',27) )     
        
        prior_temp=ncS['temp'][1,-1].data
        prior_temp[prior_temp==0]=np.nan
        prior_temp[prior_temp>1000]=np.nan

        draw_roms_pcolor(lon, lat, prior_temp, timestamp='Prior ('+t_str+')', varname='Surface temp',\
                            units=f'degree', log_scale=False, clim=(-1,32),\
                                output_path=wpth+'prior_temp/'+t_str+'.png',\
                                    cmap=plt.get_cmap('Spectral_r',27) )

        prior_salt=ncS['salt'][1,-1].data
        prior_salt[prior_salt==0]=np.nan    
        prior_salt[prior_salt>100]=np.nan    

        draw_roms_pcolor(lon, lat, prior_salt, timestamp='Prior ('+t_str+')', varname='Surface salt',\
                            units=f'psu', log_scale=False, clim=(32.5,35.5),\
                                output_path=wpth+'prior_salt/'+t_str+'.png',\
                                    cmap=cmocean.cm.haline)           
