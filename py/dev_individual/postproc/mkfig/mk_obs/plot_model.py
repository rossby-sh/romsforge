...
phytS=ncS['phytoplankton'][0,-1].data
phytS[phytS>1000]=np.nan
t=times[0]
# t_str=str(num2date(t,'seconds since 2000-1-1'))[:13]
t_str=str(num2date(t,'seconds since 2000-1-1'))[:7]
...
draw_roms_pcolor(lon, lat, phytS, timestamp=t_str, varname=title+'Mean surface phyt',\
                 units='millimole_nitrogen meter-3', log_scale=True, clim=None,\
                     output_path=wpth+'phyto/'+t_str+'.png', cmap=plt.get_cmap('jet',27)) 

    
    
tempS=ncS['temp'][0,-1].data
tempS[tempS>1000]=np.nan
draw_roms_pcolor(lon, lat, tempS, timestamp=t_str, varname=title+'Mean surface temp',\
                    units=f'degree', log_scale=False, clim=(-1,32),\
                        output_path=wpth+'temp/'+t_str+'.png',\
                        cmap=plt.get_cmap('Spectral_r',27) )

saltS=ncS['salt'][0,-1].data
saltS[saltS>1000]=np.nan
draw_roms_pcolor(lon, lat, saltS, timestamp=t_str, varname=title+'Mean surface salt',\
