...
for rstpath in rstlist:
    ...
    depth3d = compute_sigma_3d(std_h, tcline, theta_s, theta_b, nlayer, Vtransform, Vstretching) * -1
    depth3d = depth3d[::-1, :, :]
     # chlorophyll이 필요한 경우 phytoplankton에서 미리 계산
    need_chl = any('chlorophyll' in d for d in stdvars)
    chl_from_phyto = None
    ### ======== Calc chl with phyt ================================
    print('!!! SHJO: calc chl wich phyt !!!')
    if need_chl:
        # phytoplankton 정보를 찾아서 먼저 보간
        phyto_info = next(d for d in stdvars if 'phytoplankton' in d)
        phyto_name = 'phytoplankton'
        grid_type, vartype = phyto_info[phyto_name]

        print(f' - Preloading phytoplankton for derived chlorophyll')

        phyto_data = np.squeeze(rst.variables[phyto_name][:])
        if len(phyto_data.shape) == 3:
            phyto_data = phyto_data[::-1, :, :]
        ...
        phyto_interp = interpolate_parallel(romlon, romlat, phyto_data, std_lon_grid, std_lat_grid, n_jobs=-1)

        if vartype == 3:
            chl_from_phyto = vertical_interpolation_parallel(depth3d, std_h, phyto_interp, std_depth, n_jobs=-1)
            chl_from_phyto = chl_from_phyto*0.02*6.625*12
        else:
            chl_from_phyto = phyto_interp*0.02*6.625*12
    ### ======== END Calc chl with phyt ================================
    ...
    for var_dict in stdvars:
        ...
        ### ======================== Save chl ==============================
        # chlorophyll은 직접 로드하지 않고 phytoplankton에서 유도
        if var_name == 'chlorophyll' and chl_from_phyto is not None:
            print(f' - Derived variable: {var_name} from phytoplankton')
            std_array = chl_from_phyto
