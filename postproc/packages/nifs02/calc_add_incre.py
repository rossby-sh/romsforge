...
with Dataset(cfg["ininame_src"], maskandscale=True) as nc_raw:
    ...
    no3_post = nc.get('NO3', 0, slice(None))
...
#    iron_post = nc.get('iron', 0, slice(None))
    ...
    no3_prior = nc.get('NO3', 1, slice(None))
    phyt_prior = nc.get('phytoplankton', 1, slice(None))
    zoop_prior = nc.get('zooplankton', 1, slice(None))
    detr_prior = nc.get('detritus', 1, slice(None))
#    phytfe_prior = nc.get('phytoplanktonFe', 1, slice(None))
#    iron_prior = nc.get('iron', 1, slice(None))
    ...
    phyt = phyt_post - phyt_prior 
    zoop = zoop_post - zoop_prior 
    detr = detr_post - detr_prior 
    no3 = no3_post - no3_prior 
#    phytFe = phytfe_post - phytfe_prior 
#    iron = iron_post - iron_prior 
...
with Dataset(cfg["ininame_dst"], maskandscale=True) as nc_raw:
    ...
    phyt_hres = nc.get('phytoplankton', 0, slice(None))
    zoop_hres = nc.get('zooplankton', 0, slice(None))
    detr_hres = nc.get('detritus', 0, slice(None))
    no3_hres = nc.get('NO3', 0, slice(None))
#    phytfe_hres = nc.get('phytoplanktonFe', 0, slice(None))
#    iron_hres = nc.get('iron', 0, slice(None))
...
add_phyt_incre = phyt_hres + getattr(field2,'phytoplankton')
setattr(field2,"phytoplankton",add_phyt_incre)
add_zoop_incre = zoop_hres + getattr(field2,'zooplankton')
setattr(field2,"zooplankton",add_zoop_incre)
add_no3_incre = no3_hres + getattr(field2,'NO3')
setattr(field2,"NO3",add_no3_incre)
add_detr_incre = detr_hres + getattr(field2,'detritus')
setattr(field2,"detritus",add_detr_incre)

#add_phytfe_incre = phytfe_hres + getattr(field2,'phytoplanktonFe')
#setattr(field2,"phytoplanktonFe",add_phytfe_incre)
#add_iron_incre = iron_hres + getattr(field2,'iron')
#setattr(field2,"iron",add_iron_incre)
