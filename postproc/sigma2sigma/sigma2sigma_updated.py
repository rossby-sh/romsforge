...
# NOTE:
# - Source 3D velocity is provided on RHO points in geographic components:
#     u_eastward (m/s), v_northward (m/s)
# - We therefore DO NOT do uv->rho conversion, and DO NOT rotate by grid angle.
# - We only stagger rho -> u/v at the end to write ROMS initial conditions.
...
with Dataset(cfg["ininame_src"], maskandscale=True) as nc_raw:
    ...
    # 3D velocity on rho points in geographic components
    u_eastward = nc.get('u_eastward', idt, slice(None)).squeeze()
    v_northward = nc.get('v_northward', idt, slice(None)).squeeze()
    ...
    field = ut.ConfigObject(
        ...
        NO3=NO3,
        detritus=detr,
        # placeholders (will be created later after staggering)
        u=None, v=None, ubar=None, vbar=None,
    )
...
for name in [
    ...
    "zooplankton", "phytoplankton", "NO3", "detritus"
]:
...
# 수직보간 끝난 뒤 Rmask 적용
# - 스칼라는 여기서 적용
for name in ["temp", "salt", "zooplankton", "phytoplankton", "NO3", "detritus"]:
    if hasattr(field2, name):
        A = getattr(field2, name)
        A[:, Rmask == 0] = 0.0
        setattr(field2, name, A)

# - 벡터(geo, rho)도 land를 0 처리해서 이후 staggering/평균에서 오염 방지
for name in ["u_eastward", "v_northward"]:
    if hasattr(field2, name):
        A = getattr(field2, name)
        A[:, Rmask == 0] = 0.0
        setattr(field2, name, A)
...
# --- Stagger rho (geo E/N) -> ROMS u/v points (still geo components) ---
mask_u = grd_dst["mask_u"][:]
mask_v = grd_dst["mask_v"][:]

Ugeo_rho = field2.u_eastward
Vgeo_rho = field2.v_northward

u_on_u = pu.rho2uv_nanaware(Ugeo_rho, pos="u")
v_on_v = pu.rho2uv_nanaware(Vgeo_rho, pos="v")

# 마지막에만 mask_u/v 적용
u_on_u = np.where(mask_u == 1, u_on_u, 0.0)
v_on_v = np.where(mask_v == 1, v_on_v, 0.0)

# optional horizontal flooding on staggered grids
...
# --- Barotropic from 3D u,v and Hz (consistent with staggered u/v) ---
...
with Dataset(cfg["ininame_dst"], mode='a') as nc:
    for varname, arr in vars(field2).items():
        if arr is None:
            continue
        if varname not in nc.variables:
            # skip anything not present in the target template
            continue
        nc[varname][0] = arr
