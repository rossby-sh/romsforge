...
# 파일 열기
obs_file = '/home/shjo/data/nifs01_obs_EN4.nc'
ds = nc.Dataset(obs_file)

Gnm='/home/shjo/data/NWP4_grd.nc'
ncG=nc.Dataset(Gnm)
...
X = ds.variables['obs_Xgrid'][:]
Y = ds.variables['obs_Ygrid'][:]
Z = ds.variables['obs_Zgrid'][:]
...
# type == 9 (Phyt)만 선택
if type is not None:
    sst_idx = np.where(type == 6)
    X, Y, val, err, time_dates, depth = X[sst_idx], Y[sst_idx], val[sst_idx], err[sst_idx], time_dates[sst_idx], Z[sst_idx]


print(time_dates)
print(len(sst_idx[0]))
import datetime as dt
idt=np.where( (time_dates>=dt.datetime(2023,1,1) ) & (time_dates<dt.datetime(2023,1,9)) )
print(len(idt[0]))
idt=np.where( (time_dates>=dt.datetime(2023,1,1) ) & (time_dates<dt.datetime(2023,1,2)) )
print(len(idt[0]))

A = [int(i) for i in Z[idt]]
ZZ=np.unique(A)
print(len(ZZ))
...
for t in unique_times:
    ...
    # 2. scatter
    sc = ax.scatter(
        lon_rho[X[idx],Y[idx]], lat_rho[X[idx],Y[idx]], c=val_log,
        cmap=cmap, norm=norm,
