...
flist=[Spth+i for i in os.listdir(Spth) if i.startswith('NWP4_avg')]
...
def draw_roms_pcolor(lon2d, lat2d, var2d, timestamp=None,
    ...
    ct=ax.contour(lon2d, lat2d, val_plot,colors='k',linestyles='-',extend='both',linewidths=0.8,alpha=0.85,levels=[4,8,12,16,20,24,28])
...
for f in flist:
    ...
    with Dataset(f) as ncS:
        ...
        for i in range(len(times)):
            ...
            draw_roms_pcolor(lon, lat, phytS, timestamp=t_str, varname='Control phyto',\
                             units='millimole_nitrogen meter-3', log_scale=True, clim=None,\
                                 output_path=wpth+'all_phyt/'+t_str+'.png', cmap=plt.get_cmap('jet',27)) 
            
                
                
            tempS=ncS['temp'][i,-1].data
            tempS[tempS>1000]=np.nan
            draw_roms_pcolor(lon, lat, tempS, timestamp=t_str, varname='Control SST',\
                                units=f'degree', log_scale=False, clim=(-1,32),\
                                    output_path=wpth+'all_temp/'+t_str+'.png',\
                                    cmap=plt.get_cmap('Spectral_r',27) )
            
            saltS=ncS['salt'][i,-1].data
            saltS[saltS>1000]=np.nan
            draw_roms_pcolor(lon, lat, saltS, timestamp=t_str, varname='Control SSS',\
                                units=f'psu', log_scale=False, clim=(32.5,35.5),\
                                    output_path=wpth+'all_salt/'+t_str+'.png',\
