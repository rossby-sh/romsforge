...
for t in unique_times[90:98]:
    ...
    # === log10 변환된 오차 값 ===
    val_raw = err[idx]
    val_clipped = np.clip(val_raw, 1e-5, None)  # 0 이하 방지
    ...
    ax.set_title(f'Phyt Observation Error (log) - {t.strftime("%Y-%m-%d")}', fontsize=16, pad=10)
    ...
    cbar.set_label('log(Error) [log(mmol m⁻³)]', fontsize=12)

    # tick 위치와 라벨 (실제 값으로 표시되도록 log tick)
    tick_vals = [-2, -1.52, -1, -0.52]  # log10 scale
    tick_labels = ['0.01', '0.03', '0.1', '0.3']
    ...
    plt.tight_layout()
    plt.savefig(f'D:/shjo/obs_phyt_fig/e/phyt_err_log_{t.strftime("%Y%m%d")}.png', dpi=200)
    plt.show()
