...
# ROMS grid 열기
ncG = Dataset(pthG + 'NWP4_grd_3_10m_LP.nc')
lon_rho = ncG['lon_rho'][:]
lat_rho = ncG['lat_rho'][:]

# Observation 파일 출력 경로
outputFile = 'D:/shjo/ROMS_inputs/NWP4_3_10_LP/obs_sst_30km_N20.nc'
...
def writeData(outputFile,obs_lat,obs_lon,obs_value,Nobs,survey_time,obs_time,obs_Xgrid,obs_Ygrid,
    ...
    if firstIteration is True:
        ...
        f1.description="This is a obs file for OSTIA SST, MODIS PHYT, KODC temp & salt"
...
for t in range(len(ds.time)):
    ...
    obs_value = (sst - 273.15).values[::6,::6].flatten()
    
    obs_error = error.values[::6,::6].flatten()
    # obs_error=obs_error**2
    obs_error=np.ones(len(obs_value))*0.3**2
    
    obs_lat = lat2d[::6,::6].flatten()
    obs_lon = lon2d[::6,::6].flatten()
