...
Spth='D:/shjo/MCC/nifs_jun_srf3/ROMS_F_ERA5_3hourly_250531-250701.nc'

title='OSTIA SST'

wpth='D:/shjo/MCC/nifs_jun_srf3/figs/frc/'
...
def draw_roms_pcolor(lon2d, lat2d, var2d, timestamp=None,
    ...
    ct=ax.contour(lon2d, lat2d, val_plot,colors='k',linestyles='-',linewidths=0.8,alpha=0.85,levels=[10,30,50,70,90])
...
lon,lat=ncS['lon'][:].data,ncS['lat'][:].data
...
for i in range(len(times)):
    ...
    t=times[i]
    t_str=str(num2date(t,'days since 2000-01-01'))[:13]
    ...
    # tempS=ncS['Qair'][i].data
    # tempS[tempS<-1000]=np.nan
    # draw_roms_pcolor(lon, lat, tempS, timestamp=t_str, varname="Frc Qair",\
    #                     units=f'%', log_scale=False, clim=(0,100),\
    #                         output_path=wpth+'qair/'+t_str+'.png',\
    #                         cmap=plt.get_cmap('Spectral_r',27) )
    
        
        
            
    tempS=ncS['srf'][i].data
    tempS[tempS<-1000]=np.nan
    draw_roms_pcolor(lon, lat, tempS, timestamp=t_str, varname="Frc srf",\
                        units=f'W/m^2', log_scale=False, clim=(0,1000),\
                            output_path=wpth+'srf/'+t_str+'.png',\
                            cmap=plt.get_cmap('Spectral_r',27) )
    
        
    lwrad_down=ncS['lwrad_down'][i].data
    lwrad_down[lwrad_down<-1000]=np.nan
    draw_roms_pcolor(lon, lat, lwrad_down, timestamp=t_str, varname="Frc lwrad_down",\
                        units=f'W/m^2', log_scale=False, clim=(0,500),\
                            output_path=wpth+'lwrad_down/'+t_str+'.png',\
                            cmap=plt.get_cmap('Spectral_r',27) )
