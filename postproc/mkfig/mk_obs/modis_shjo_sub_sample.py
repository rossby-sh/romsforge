...
outputFile = 'D:/shjo/ROMS_inputs/obs/obs_phyt_9km.nc'
...
ds_combined = xr.concat(datasets, dim='time').sel(
    lon=slice(110,160), lat=slice(55,5))
...
#for t in range(5):
    ...
    obs_value = chl.values[::1,::1].flatten()/(0.02*6.625*12)  # 단위 변환: mg/m^3 → mmol/m^3
    obs_lat = lat2d[::1,::1].flatten()
    obs_lon = lon2d[::1,::1].flatten()
    ...
    obs_error = np.maximum(obs_value * 0.3, 1e-5)
