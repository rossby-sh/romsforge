...
def create_roms(cfg, grd, initime_num,time_ref, bio_model=None, ncFormat='NETCDF3_CLASSIC'):

    vstretching, vtransform = cfg.vertical_dst.vstretching, cfg.vertical_dst.vtransform
    theta_s, theta_b = cfg.vertical_dst.theta_s, cfg.vertical_dst.theta_b
    tcline, layer_n = cfg.vertical_dst.tcline, cfg.vertical_dst.layer_n
    ...
    mode = 'w' if cfg.force_write else 'x'
    try:
        ncfile = Dataset(cfg.ininame_dst, mode=mode, format=ncFormat)
    except FileExistsError:
        print(f"--- [!ERROR] {cfg.ininame} already exists and force_write=False ---")
        return 1
    ...
    ncfile.title = cfg.global_attrs.title
    ncfile.src_clim_file = cfg.ininame_src
    ncfile.dst_clim_file = cfg.ininame_dst

    ncfile.src_grd_file = cfg.grdname_src
    ncfile.dst_grd_file = cfg.grdname_dst

    ncfile.type = cfg.global_attrs.type
    ncfile.history = f"Created on {dt.datetime.now().isoformat()}"

    ncfile.close()
    print(f"--- [+] Initial file created: {cfg.ininame_dst} ---")
    return 0
