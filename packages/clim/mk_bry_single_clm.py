...
with step("[02] Load climatology file"):
    ...
    ogcm = tl.load_ogcm_metadata(clim_file, cfg.ogcm_var_name)

# --- [03] Create initial NetCDF file ---
with step("[03] Create boundary NetCDF"):
    status = cn.create_bry_clm(cfg, grd, clm_time, bio_model=cfg.bio_model_type, ncFormat=cfg.ncformat)
    if status:
...
nc_clim = tl.MaskedNetCDF(Dataset(cfg.ogcm_name, mode="r", maskandscale=True))

for i, t in enumerate(time_ref_list):
    tag = f"month={i+1:02d}"

    with step("[07] Load OGCM fields", ts=tag):
        zeta = nc_clim.get(cfg.ogcm_var_name['zeta'], i, idy, idx)
        temp = nc_clim.get(cfg.ogcm_var_name['temperature'], i, slice(None), idy, idx)
        salt = nc_clim.get(cfg.ogcm_var_name['salinity'], i, slice(None), idy, idx)
        u    = nc_clim.get(cfg.ogcm_var_name['u'], i, slice(None), idy, idx)
        v    = nc_clim.get(cfg.ogcm_var_name['v'], i, slice(None), idy, idx)
