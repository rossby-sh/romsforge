#!/bin/bash
#
# svn $Id: build.bash 720 2014-03-13 22:47:51Z arango $
#::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
...
#export       MY_ROMS_SRC=${MY_ROOT_DIR}/branches/arango
 export       MY_ROMS_SRC=/home/shjo/roms/roms_820_nemucsc_latest
  ...
  case "$FORT" in
    ifort )
      if [ "${which_MPI}" = "mpich" ]; then
        ...
        export PATH=/usr/local/mpi/intel21/mvapich2-2.3.4/bin:$PATH
      elif [ "${which_MPI}" = "openmpi" ]; then
        export PATH=/usr/local/mpi/intel21/openmpi-5.0.0/bin:$PATH
      fi
      ;;
...
if [ -n "${USE_MY_LIBS:+1}" ]; then
  case "$FORT" in
    ifort )
      ...
      if [ -n "${USE_NETCDF4:+1}" ]; then
        if [ -n "${USE_PARALLEL_IO:+1}" ] && [ -n "${USE_MPI:+1}" ]; then
          ...
          export       NC_CONFIG=/usr/local/netcdf/4.6.1_intel21/bin/nc-config
          export   NETCDF_INCDIR=/usr/local/netcdf/4.6.1_intel21/include
        fi
      else
        export     NETCDF_INCDIR=/usr/local/netcdf/3.6.3_intel19/include
        export     NETCDF_LIBDIR=/usr/local/netcdf/3.6.3_intel19/lib
      fi
      ;;
    ...
    pgi )
      ...
      export       ARPACK_LIBDIR=/home/jhlee/model/ROMS_latest/Lib/ARPACK
      if [ -n "${USE_MPI:+1}" ]; then
        if [ "${which_MPI}" = "mpich" ]; then
          ...
          export  PARPACK_LIBDIR=/home/jhlee/model/ROMS36/Lib/ARPACK
        elif [ "${which_MPI}" = "openmpi" ]; then
          ...
          export  PARPACK_LIBDIR=/home/jhlee/model/ROMS_latest/Lib/ARPACK
        fi
      fi
...
export EXE_DIR=${MY_PROJECT_DIR}
mv ${EXE_DIR}/oceanM ${EXE_DIR}/execM_nl
