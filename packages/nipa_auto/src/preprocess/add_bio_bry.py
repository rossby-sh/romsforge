...
with step("[01] Load configuration and input metadata"):
    ...
    info(f"daily_files={cfg.bio_ogcm_name}")
...
with step("[03] Create initial NetCDF"):
    status = cn.create_bry_bio(
        cfg, relative_time, bio_model=cfg.bio_model_type,bio_yaml=bio_yml
    )
    if status:
        raise RuntimeError("create_bry_bio() returned error")
        ...
        for var in bry_vars:
            for direction in ("north", "south", "east", "west"):
                ...
                if vname not in nc.variables:
                    raise RuntimeError(
                        f"[ERR] variable missing in bry file: {vname}\n"
