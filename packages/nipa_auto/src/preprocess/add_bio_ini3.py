...
#    """
#    bio_ogcm_name = cfg.bio_ogcm_name
...
with step("[01] Load configuration and input metadata"):
    ...
    ogcm = tl.load_ogcm_metadata(bio_ogcm_name, cfg.bio_ogcm_var_name)
    ...
    info(f"ogcm(daily)={bio_ogcm_name}")
    info(f"bio_model={cfg.bio_model_type}")

#with step("[02] Load daily ogcm file"):
#    assert os.path.exists(bio_ogcm_name), f"Daily OGCM file not found: {bio_ogcm_name}"
#    relative_time = date2num(cfg.initdate, cfg.time_ref)
#    info(f"Loaded daily ogcm file: {os.path.basename(bio_ogcm_name)}")
...
with step("[02] Time index matching & relative time"):
    tinfo = collect_time_info_legacy(bio_ogcm_name, cfg.bio_ogcm_var_name['time'], str(cfg.initdate))
    _, idt, _, _ = tinfo[0]
...
with step("[03b] Load bio vars (YAML)"):
    ...
    info(f"bio_yaml={bio_yml}")
    info(f"ini_vars={ini_vars}")

with step("[03] Create initial NetCDF"):
    status = cn.create_ini_bio(
        cfg, bio_model=cfg.bio_model_type,bio_yaml=bio_yml
    )
    if status:
        raise RuntimeError("create_ini_bio() returned error")
...
nc_clim = tl.MaskedNetCDF(Dataset(bio_ogcm_name, mode="r", maskandscale=True))
    ...
    with Dataset(cfg.ininame, 'a') as nc:
        ...
        for varname in ini_vars:
            if varname not in nc.variables:
                raise RuntimeError(
                    f"[ERR] variable missing in ini file: {varname}\n"
