...
with step("[01] Load configuration and input metadata"):
    ...
    # cfg.bio_ogcm_name is now a DAILY DIRECTORY
...
with step("[11] z→σ interpolation"):
    ...
    # list 유지 (너 스타일)
    zvars = [
        "NO3", "PO4", "NH4", "TIC", "oxygen",
        "chlorophyll", "phytoplankton", "zooplankton", "alkalinity",
        "SdetritusC", "LdetritusC", "RdetritusC", "SdetritusN", "RdetritusN"
    ]
...
with step("[12] Write variables", out=ellipsis(cfg.ininame)):
    with Dataset(cfg.ininame, 'a') as nc:
        # ini가 이미 만들어져 있다는 전제에서, 시간도 맞춰주고 싶으면 아래 두 줄을 켜도 됨
