...
for t in unique_times[90:98]:
    ...
    ax.set_title(f'Phyt Observation - {t.strftime("%Y-%m-%d %H:%M:%S")}', fontsize=14)
    ...
    # 관측 산점도
    val_max=np.max(val[idx])
    idV=np.argmax(val[idx])
    ...
    sc = ax.scatter(lon[idx], lat[idx], c=val[idx], cmap='jet', s=10,vmin=0, vmax=5,
                    edgecolor=None, transform=ccrs.PlateCarree())

    # 컬러바
    cbar = plt.colorbar(sc, ax=ax, orientation='vertical', pad=0.06, shrink=.8)
    cbar.set_label('Phyt Observation Value (mmol m-3)', fontsize=12)
