...
INST_FILE  = config["output"]["base_dir"]+'/tmp_frc/inst_20250630-20250801.grib'
ACCUM_FILE = config["output"]["base_dir"]+'/tmp_frc/accum_20250630-20250801.grib'
Q1000_FILE = config["output"]["base_dir"]+'/tmp_frc/q1000_20250630-20250801.grib'
W_FILE     = config["output"]["base_dir"]+"tmp.nc"
MY_TIME_REF = config["time"]["ref_time"]   # 예: "hours since 2000-01-01 00:00:00"

print(INST_FILE)

if not INST_FILE or not ACCUM_FILE:
    raise RuntimeError("config_era5.yml에 input_inst, input_accum 경로를 설정해줘.")
...
qair_values = Q["q"]["data"] if "q" in Q else None     # kg/kg @ 1000hPa (없으면 None)
